"""
Timer Overlay Module for KzBuilder 3.3.4
...
class TimerOverlay:
    ...
    def _build_ui(self):
        ...
        self.msg_container.pack(fill='both', expand=True, padx=4, pady=1)
        ...
        self.separator.pack(fill='x', padx=4, pady=1)
        self._bg_widgets.append(('border', self.separator))

        # Cycle timer label (bottom)
        self.timer_label = tk.Label(
            self.frame, text="", font=('Segoe UI', self.font_size + 4, 'bold'),
            fg='#888888', bg=self.BG_INNER, padx=5, pady=0, anchor='w'
        )
    ...
    def _create_message_row(self, row_num):
        ...
        timer_label = tk.Label(
            frame, text="", font=('Segoe UI', self.font_size, 'bold'),
    ...
    def toggle_lock(self):
        ...
        if self.is_locked:
            self.lock_indicator.config(text="🔒", fg='#555555')
            self.resize_handle.config(fg=self.BG_INNER)  # Hide
            self._set_click_through(True)
        else:
            self.lock_indicator.config(text="🔓", fg='#555555')
            self.resize_handle.config(fg='#444444')  # Show
    ...
    def set_font_size(self, size):
        ...
        msg_font = ('Segoe UI', self.font_size, 'bold')
        timer_font = ('Segoe UI', self.font_size + 4, 'bold')
